...
tener("click", function () {
  ...
  function openRazorpay(initData) {
    ...
    var rzp = new Razorpay({
      ...
      handler: function (response) {
        ...
        postAS("service_pay_complete", {
          ...
          if (!res || res.ok !== true) {
            setBusy(payBtn, false, payBtnLabel());
            showErr((res && res.message) || "Payment succeeded but we could not record it. Please contact support.");
            return;
          }
          if (paidEmailEl)   paidEmailEl.textContent = currentEmail;
          if (paidInvoiceEl) paidInvoiceEl.textContent = res.invoiceNumber || "(emailed)";
    ...
    rzp.on("payment.failed", function (resp) {
      setBusy(payBtn, false, payBtnLabel());
      showErr("Payment failed: " + (resp.error && resp.error.description ? resp.error.description : "Please try again."));
    });
    rzp.open();
  }
})();
